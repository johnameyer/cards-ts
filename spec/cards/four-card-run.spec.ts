...
describe('FourCardRun', () => {
    ...
    describe('#liveCards', () => {
        it('should contain live cards', () => {
            ...
            [J, Card.fromString('10H')].forEach((card) => {
                expect(liveCards.find((x) => x.equals(card))).to.be.ok;
            });
            //use member

            run = new FourCardRun([JH, QH, KH, J]);
            liveCards = [...run.liveCards()];
            [J, Card.fromString('10H'), AH].forEach((card) => {
                expect(liveCards.find((x) => x.equals(card))).to.be.ok;
            });
        });
    });
