    ...
    validateEvent: {
        ...
        'turn-response': {
            validators: [
                ...
                EventHandler.validate('Must follow suit if possible', (controllers, source, { card }) => controllers.trick.currentTrick[0]?.suit !== undefined && card.suit !== controllers.trick.currentTrick[0].suit && controllers.hand.get(source).some(card => card.suit === controllers.trick.currentTrick[0]?.suit)),
            ],
