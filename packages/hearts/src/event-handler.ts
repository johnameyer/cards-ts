        ...
        validateEvent: {
            validators: [
                ...
                EventHandler.validate(
                    'Cannot pass same card multiple times',
                    (controllers, source, { cards }) => cards.filter(distinct).length !== controllers.params.get().numToPass,
                ),
                EventHandler.validate('Can only pass cards that are in hand', (controllers, source, { cards }) =>
                    cards.some(card => controllers.hand.get(source).find(card.equals.bind(card)) === undefined),
                ),
            ],
            fallback: (controllers, source) => new PassResponseMessage(controllers.hand.get(source).slice(0, controllers.params.get().numToPass)),
        },
        merge: [EventHandler.removeWaiting('waiting'), (controllers, sourceHandler, incomingEvent) => controllers.passing.setPassedFor(sourceHandler, incomingEvent.cards)],
    },
    'turn-response': {
        ...
        validateEvent: {
            validators: [
                ...
                EventHandler.validate(
                    'Cannot give points on the first round',
                    (controllers, source, { card }) => controllers.trick.tricks === 0 && (card.suit === Suit.HEARTS || card.equals(QS)),
                ),
                EventHandler.validate(
                    'Blood has not been shed yet',
                    (controllers, source, { card }) =>
                        controllers.trick.currentTrick[0] === undefined && card.suit === Suit.HEARTS && controllers.trickPoints.get().every(point => point === 0),
                ),
                EventHandler.validate(
                    'Must follow suit if possible',
                    (controllers, source, { card }) =>
                        controllers.trick.currentTrick[0]?.suit !== undefined &&
                        card.suit !== controllers.trick.currentTrick[0].suit &&
                        controllers.hand.get(source).some(card => card.suit === controllers.trick.currentTrick[0]?.suit),
                ),
            ],
            fallback: (controllers, source, { card }) => {
                const fallbackCard =
                    controllers.hand.get(source).filter(card => card.suit === controllers.trick.currentTrick[0]?.suit)[0] ||
                    controllers.hand.get(source).filter(card => card.suit !== Suit.HEARTS)[0] ||
