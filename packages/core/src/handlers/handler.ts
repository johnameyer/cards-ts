...
export type HandlerAction<HandlerData, ResponseMessage extends Message, Vargs extends any[] = any[]> = (
    this: any,
    gameState: HandlerData,
    response: HandlerResponsesQueue<ResponseMessage>,
    ...args: Vargs
) => void | Promise<void>;
...
export class HandlerChain<Handlers extends { [key: string]: any[] }, HandlerData extends Serializable, ResponseMessage extends Message> {
     ...
     */
    call<Method extends keyof Handlers>(
        method: Method,
        ...args: Parameters<HandlerAction<HandlerData, ResponseMessage, Handlers[Method]>>
    ): ReturnType<HandlerAction<HandlerData, ResponseMessage, Handlers[Method]>> {
