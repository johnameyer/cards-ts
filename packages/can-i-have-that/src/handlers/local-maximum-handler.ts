...
export class LocalMaximumHandler extends GameHandler {
    handleWantCard = (
        { hand, melds: { melds }, players: { position }, canIHaveThat: { round }, params: { rounds }, deck: { deckCard }, turn, ask }: HandlerData,
    ...
    handleTurn = (
        { hand, melds: { melds }, players: { position }, canIHaveThat: { round }, params: { rounds } }: HandlerData,
        responsesQueue: HandlerResponsesQueue<DiscardResponseMessage | GoDownResponseMessage | PlayResponseMessage>,
    ) => {
        const currentRound = rounds[round];
        if (melds[position].length === 0) {
            ...
            const without = (arr: Card[], card: Card) => {
                ...
                result.splice(arr.indexOf(card), 1);
                return result;
            };
