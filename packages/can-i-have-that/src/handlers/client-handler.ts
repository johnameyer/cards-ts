...
export abstract class ClientHandler extends GameHandler {
    public abstract handleWantCard: (
        {
            hand,
            melds: { melds },
            players: { position },
            canIHaveThat: { whoseAsk, round },
            params: { rounds },
            data,
        }: HandlerData,
        queue: HandlerResponsesQueue<WantCardResponseMessage>,
    ) => Promise<void>;

    handleTurn = async (
        { hand, melds: { melds }, players: { position }, canIHaveThat: { round }, params: { rounds }, data }: HandlerData,
        responsesQueue: HandlerResponsesQueue<GoDownResponseMessage | DiscardResponseMessage | PlayResponseMessage>,
    ): Promise<void> => {
    ...
    async askToPlayOnRun(run: Meld, hand: Card[], responsesQueue: HandlerResponsesQueue<PlayResponseMessage>, data: unknown) {
        if (run instanceof ThreeCardSet) {
            ...
            cards.forEach(toRemove =>
                hand.splice(
                    hand.findIndex(card => toRemove.equals(card)),
                    1,
                ),
            );
            ...
            for (const card of cards) {
                ...
                hand.splice(
                    hand.findIndex(other => card.equals(other)),
                    1,
                );
            }
            responsesQueue.push(new PlayResponseMessage(oldMeld, cards, run));
