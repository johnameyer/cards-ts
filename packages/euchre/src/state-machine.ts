...
import { loop, sequence, handleRoundRobinFirst, handleSingle, handleRoundRobin, game, Machine, NestedMachine } from '@cards-ts/state-machine';
import { Named } from '@cards-ts/state-machine/lib/sequence.js';

const hasBidder = (controllers: Controllers) => controllers.euchre.bidder !== undefined;

const nameTrump: Named<NestedMachine<Controllers>> = {
    name: 'bidding',
    start: 'orderUp',
    states: {
        dealOut: {
