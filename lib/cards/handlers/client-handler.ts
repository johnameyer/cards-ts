...
export abstract class ClientHandler implements Handler {
    public abstract async wantCard(card: Card, isTurn: boolean, {hand, played, position, round, gameParams: {rounds}}: HandlerData): Promise<[boolean, HandlerData?]>;
    ...
    : Promise<{ toDiscard: Card | null, toPlay: Run[][] } | null> {
        ...
        if (played[position].length === 0) {
            if (await this.wantToGoDown(hand)) {
                ({toPlay: toPlay[position], hand} = await this.goDown(hand, currentRound, last));
            }
        }
    ...
    private async playOnOthers(hand: Card[], played: Run[][]) {
        while (await this.wantToPlay(played.reduce(flatten, []), hand)) {
            const runToPlayOn = await this.whichPlay(played.reduce(flatten, []), hand);
            await this.askToPlayOnRun(runToPlayOn, hand);
        }
    }

    private async goDown(hand: Card[], roun: (3 | 4)[], last: boolean): Promise<{toPlay: Run[], hand: Card[]}> {
        let cardsLeft: Card[] = hand.slice();
        const toPlay = [];
        for (const num of roun) {
            const newRun = await this.createRun(num, cardsLeft);
            if(!newRun) {
                return { toPlay: [], hand };
            }
            toPlay.push(newRun);
    ...
    private async discard(cardsLeft: Card[], roun: (3 | 4)[], played: Run[][]) {
        let live: Card[];
        ...
        const toDiscard = Card.fromObj(await this.discardChoice(cardsLeft, live));
        return toDiscard;
    }

    private async askToPlayOnRun(run: Run, hand: Card[]) {
        if (run instanceof ThreeCardSet) {
            const cards: Card[] = await this.cardsToPlay(hand, run);
            if (!cards) {
                return;
            }
            ...
            const cards = await this.cardsToPlay(hand, run);
            
            //TODO add check for when to ask
            const moveToTop = await this.moveToTop();
    ...
    async createRun(num: 3 | 4, cardsLeft: Card[]) {
        ...
        if (num === 3) {
            ...
            for (let wild of wilds) {
                let position = await this.insertWild(run, wild);
    ...
    abstract async selectCards(cardsLeft: Card[], num: 3 | 4): Promise<Card[]>;

    abstract async cardsToPlay(hand: Card[], run: Run): Promise<Card[]>;

    abstract async moveToTop(): Promise<boolean>;
    
    abstract async wantToPlay(played: Run[], hand: Card[]): Promise<boolean>;

    abstract async whichPlay(runOptions: Run[], hand: Card[]): Promise<Run>;

    abstract async wantToGoDown(hand: Card[]): Promise<boolean>;

    abstract async discardChoice(cardsLeft: Card[], live: Card[]): Promise<Card>;

    abstract async insertWild(run: Card[], wild: Card): Promise<number>;
